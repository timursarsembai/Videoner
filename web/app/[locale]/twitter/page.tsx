...
export async function generateMetadata({
  ...
  const strings = translations[lang].platforms.twitter;
  const title = `${strings.titleLine1} ${strings.titleLine2} | Videoner`;
  const description = strings.pageDescription;
  const canonical = `https://videoner.download${localizedHref(lang, "/twitter")}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        en: "https://videoner.download/twitter",
        ru: "https://videoner.download/ru/twitter",
        es: "https://videoner.download/es/twitter",
        "x-default": "https://videoner.download/twitter",
      },
    },
  };
}
...
export default async function PlatformPage({
  ...
  const softwareApplicationSchema = {
    ...
    operatingSystem: "Any",
    url: canonical,
    description: translations[lang].platforms.twitter.pageDescription,
    offers: {
      "@type": "Offer",
      price: "0",
      priceCurrency: "USD",
  ...
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: translations[lang].platforms.twitter.faq.map((item) => ({
      "@type": "Question",
      name: item.q,
      acceptedAnswer: {
        "@type": "Answer",
