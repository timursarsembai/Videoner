...
export async function generateMetadata({
  ...
  const strings = translations[lang].platforms.threads;
  const title = `${strings.titleLine1} ${strings.titleLine2} | Videoner`;
  const description = strings.pageDescription;
  const canonical = `https://videoner.download${localizedHref(lang, "/threads")}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        en: "https://videoner.download/threads",
        ru: "https://videoner.download/ru/threads",
        es: "https://videoner.download/es/threads",
        "x-default": "https://videoner.download/threads",
      },
    },
  };
}
...
export default async function PlatformPage({
  ...
  const softwareApplicationSchema = {
    ...
    operatingSystem: "Any",
    url: canonical,
    description: translations[lang].platforms.threads.pageDescription,
    offers: {
      "@type": "Offer",
      price: "0",
      priceCurrency: "USD",
  ...
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: translations[lang].platforms.threads.faq.map((item) => ({
      "@type": "Question",
      name: item.q,
      acceptedAnswer: {
        "@type": "Answer",
