...
export class AlertService {
  ...
  async notifyProxyDown(
    ...
    const text =
      ...
      this.proxyAdvice(reason);

    await this.send(text);
  }

  // Совет зависит от кода ответа, иначе он уводит в сторону. 22.09.2026 алерт
  // трижды за сутки советовал купить новый прокси, хотя аренда была оплачена до
  // 18.10, трафика оставалось 94 ГБ, а пароль принимался. Причина была другая:
  // тот же ISP-прокси прописан в KSAVISA_MOFA_PROXY на сервере в Алматы, и
  // одновременно с двух адресов источника он не работает — когда
  // ksavisa-worker брал канал, Videoner получал 403, и наоборот. Полчаса ушло
  // на проверку остатка трафика и срока аренды, то есть ровно на то, что
  // советовал этот текст.
  private proxyAdvice(reason: string): string {
    if (reason.includes('407')) {
      return (
        'HTTP 407 значит, что прокси не принял логин и пароль — вот тут стоит ' +
        'смотреть аренду и учётные данные в панели IPRoyal. Если аренда кончилась, ' +
        'порядок такой: купить новый прокси, обновить YOUTUBE_PROXY_URL в Infisical ' +
        'в ОБОИХ окружениях (prod и staging), затем ./scripts/deploy.sh server ' +
        'threads-resolver и ./scripts/deploy-staging.sh server.'
      );
    }

    if (reason.includes('403')) {
      return (
        'HTTP 403 значит, что логин с паролем приняты, а доступ закрыт. Аренда и ' +
        'остаток трафика почти наверняка ни при чём — при них ответ был бы 407. ' +
        'Первым делом проверить, не занял ли канал другой сервер: тот же прокси ' +
        'стоит в KSAVISA_MOFA_PROXY на 93.171.232.110, и одновременно с двух ' +
        'адресов он не пускает. Если нужны оба сервиса, разводить их на два ' +
        'отдельных прокси — белый список адресов эту проблему не решает, ' +
        'проверено 22.09.2026.'
      );
    }

    return (
      'Проверить в панели IPRoyal аренду, остаток трафика и белый список адресов, ' +
      'а также не занял ли канал второй сервер (тот же прокси стоит в ' +
      'KSAVISA_MOFA_PROXY на 93.171.232.110). Если прокси придётся менять, порядок ' +
      'такой: обновить YOUTUBE_PROXY_URL в Infisical в ОБОИХ окружениях (prod и ' +
      'staging), затем ./scripts/deploy.sh server threads-resolver и ' +
