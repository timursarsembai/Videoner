...
export async function generateMetadata({
  ...
  const strings = translations[lang].platforms.vimeo;
  const title = `${strings.titleLine1} ${strings.titleLine2} | Videoner`;
  const description = strings.pageDescription;
  const canonical = `https://videoner.download${localizedHref(lang, "/vimeo")}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        en: "https://videoner.download/vimeo",
        ru: "https://videoner.download/ru/vimeo",
        es: "https://videoner.download/es/vimeo",
        "x-default": "https://videoner.download/vimeo",
      },
    },
  };
}
...
export default async function PlatformPage({
  ...
  const softwareApplicationSchema = {
    ...
    operatingSystem: "Any",
    url: canonical,
    description: translations[lang].platforms.vimeo.pageDescription,
    offers: {
      "@type": "Offer",
      price: "0",
      priceCurrency: "USD",
  ...
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: translations[lang].platforms.vimeo.faq.map((item) => ({
      "@type": "Question",
      name: item.q,
      acceptedAnswer: {
        "@type": "Answer",
