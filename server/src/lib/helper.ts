...
export function parseDownloadOptions<T extends DownloadKeyWord>(
  ...
  if (filter === 'mergevideo') {
    ...
    if (height && platform === 'youtube') {
      ...
      //
      // Разрешения `sd`/`hd` заранее неизвестны, поэтому нужный выбираем по
      // запрошенному качеству: до 480p — `sd`, выше — `hd`. Пропадут эти
      // форматы — цепочка ниже откатится на общий отбор, где сортировка
      // по-прежнему предпочтёт avc1.
      //
