...
export async function generateMetadata({
  ...
  const strings = translations[lang].platforms.rutube;
  const title = `${strings.titleLine1} ${strings.titleLine2} | Videoner`;
  const description = strings.pageDescription;
  const canonical = `https://videoner.download${localizedHref(lang, "/rutube")}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        en: "https://videoner.download/rutube",
        ru: "https://videoner.download/ru/rutube",
        es: "https://videoner.download/es/rutube",
        "x-default": "https://videoner.download/rutube",
      },
    },
  };
}
...
export default async function PlatformPage({
  ...
  const softwareApplicationSchema = {
    ...
    operatingSystem: "Any",
    url: canonical,
    description: translations[lang].platforms.rutube.pageDescription,
    offers: {
      "@type": "Offer",
      price: "0",
      priceCurrency: "USD",
  ...
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: translations[lang].platforms.rutube.faq.map((item) => ({
      "@type": "Question",
      name: item.q,
      acceptedAnswer: {
        "@type": "Answer",
