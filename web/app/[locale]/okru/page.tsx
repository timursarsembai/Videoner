...
export async function generateMetadata({
  ...
  const strings = translations[lang].platforms.okru;
  const title = `${strings.titleLine1} ${strings.titleLine2} | Videoner`;
  const description = strings.pageDescription;
  const canonical = `https://videoner.download${localizedHref(lang, "/okru")}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        en: "https://videoner.download/okru",
        ru: "https://videoner.download/ru/okru",
        es: "https://videoner.download/es/okru",
        "x-default": "https://videoner.download/okru",
      },
    },
  };
}
...
export default async function PlatformPage({
  ...
  const softwareApplicationSchema = {
    ...
    operatingSystem: "Any",
    url: canonical,
    description: translations[lang].platforms.okru.pageDescription,
    offers: {
      "@type": "Offer",
      price: "0",
      priceCurrency: "USD",
  ...
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: translations[lang].platforms.okru.faq.map((item) => ({
      "@type": "Question",
      name: item.q,
      acceptedAnswer: {
        "@type": "Answer",
