  ...
  case "$service" in
    web)
      ...
      for var in $used_vars; do
        ...
        #
        # "|| true" здесь обязателен, и вот почему (найдено 05.08.2026): при
        # set -euo pipefail неудачный grep роняет ВЕСЬ скрипт прямо на этом
        # присваивании. А не найтись переменная должна штатно — как раз для
        # NEXT_PUBLIC_API_URL, с которой список и начинается после sort. То
        # есть проверка бандла не «пропускала отдельные переменные», а не
        # выполнялась НИКОГДА: деплой обрывался сразу после пересоздания
        # контейнера, между "==> Проверяю..." и первым ✓. Снаружи это выглядело
        # как успешный деплой — вызывающий обычно смотрит в конец вывода, а
        # код возврата теряется, если вывод уходит в пайп (| tail и т.п.).
        # Ровно эту дыру скрипт и должен был закрывать: молча собранный без
        # build-args web (инцидент с Turnstile 23.07.2026).
        value="${!var:-$(grep -m1 "^${var}=" .env 2>/dev/null | cut -d= -f2 || true)}"
        if [ -z "$value" ]; then
          continue
        fi
        checked=$((checked + 1))
