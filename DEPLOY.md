...
Монтируется каталог `server/cookies/`, поэтому он должен существовать
(пустой файл внутри = анонимный режим):

```bash
mkdir -p server/cookies && touch server/cookies/cookies.txt
```

Позже положите туда куки по одному файлу на площадку — `youtube.txt`, `tiktok.txt`
и т.д. (см. README, раздел «Cookies»). YouTube без кук почти не работает —
его файл стоит добавить сразу.
